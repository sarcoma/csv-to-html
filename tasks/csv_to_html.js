 ...
 *
 *
 * This has been hacked together to save one file per row of the csv
 * Author: Sarcoma
 */
...
module.exports = function(grunt) {
    ...
    grunt.registerMultiTask('csv_to_html_files', 'Compile HTML files with csv & mustache/handlebars template', function() {
        ...
        this.files.forEach(function(f) {
            ...
            // Validate file accessibility, separate csv from tpl
            ...
            csv.parse(data, options.csv, function(err, rows){
                ...
                for(var i=0,l=rows.length;i<l;i++){
                    ...
                    var fname = row.name.replace(/,/g, '').split(' ').join('-').toLowerCase();
